...
import {getAuthId, getTime} from '../../../services/FirebaseAuth';
...
const Settings = ({navigation}) => {
  ...
  const [generateLinkTime, setGenerateLinkTime] = useState("")
 ...
 getAuthData()
  ...
  const getAuthData= async()=>{
    ...
    // const getLinkTime=await getTime()

    // setGenerateLinkTime(getLinkTime)

    // console.log("getTimeAll",getTimeData)
  ...
  const generateLink = async () => {
    ...
    const linkTime=moment(linkDate).format("hh:mm A")

    console.log(linkTime)
    try {
      var link = await dynamicLinks().buildShortLink(
        {
          link: `https://getsolemate.page.link/EwdR?screen=Profile&withApp=true&linkDate=${linkTime}&id=${id}`,
          domainUriPrefix: 'https://getsolemate.page.link',
          android: {
            packageName: 'com.Solmate',
            minimumVersion: '18',
