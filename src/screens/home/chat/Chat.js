...
const Chat = ({navigation, route}) => {
  ...
  const onSend = async (result, file, audioUri) => {
    ...
    // send notification to other user with message
    NotificationSender(
  ...
  const getAudioUri = async uri => {
    ...
    try {
      ...
      // store audio to firebase storage
      UploadAudioToFirebaseStorage(result, uri);
  ...
  const UploadAudioToFirebaseStorage = async (result, file) => {
    const resData = 0;
    const fileData = 0;
  ...
  // save user reaction to firabase
    ...
    <KeyboardAvoidingView
      ...
      <SafeAreaView style={commonStyles.commonMain}>
        <View style={styles.mainContainer}>
          ...
          {/*  showEndConversion View  user click three dots*/}
          {showEndConversion ? (
